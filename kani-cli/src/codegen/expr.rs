...
pub fn emit_expr(expr: &Expr) -> String {
    match expr {
        ...
        Expr::Select   { target, selector }  => format!("{}.select(\"{}\")", e(target), escape(selector)),
        Expr::First    { target, selector }  => format!("{}.first(\"{}\")", e(target), escape(selector)),
        Expr::HasClass { target, class }     => format!("{}.has_class(\"{}\")", e(target), escape(class)),
        Expr::Children { target }            => format!("{}.children()", e(target)),
        Expr::Split    { target, delimiter } => format!("{}.split(\"{}\")", e(target), escape(delimiter)),
        ...
        Expr::Replace  { target, from, to } =>
            format!("{}.replace(\"{}\", \"{}\")", e(target), escape(from), escape(to)),
        Expr::Slice { target, start, end: None } =>
            format!("{}.slice({}, None)", e(target), start),
        Expr::Slice { target, start, end: Some(end) } =>
        ...
        Expr::JsonPtr { target, pointer } =>
            format!("{}.ptr(\"{}\")", e(target), escape(pointer)),
        ...
        Expr::Fold { target, base, transform } =>
        ...
        Expr::If { condition, then, else_ } =>
            format!("Expr::if_then_else({}, {}, {})", e(condition), e(then), e(else_)),
        Expr::Let { name, value, body } =>
            format!("Expr::let_bind(\"{}\", {}, {})", escape(name), e(value), e(body)),
        ...
        Expr::Format { template, args } =>
            format!("Expr::format(\"{}\", vec![{}])", escape(template), emit_list(args)),
    }
}

fn e(expr: &Expr) -> String { emit_expr(expr) }
