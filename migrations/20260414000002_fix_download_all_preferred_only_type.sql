-- SQLite does not support ALTER COLUMN, so we recreate the manga table to
-- change download_all_preferred_only from INTEGER to BOOLEAN so that sqlx
-- can decode the column directly into a Rust bool.
...
CREATE TABLE manga_new (
    ...
    download_all_preferred_only BOOLEAN  NOT NULL DEFAULT 1,
