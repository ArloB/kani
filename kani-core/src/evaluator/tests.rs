...
mod helpers {
    ...
    pub(crate) fn binop(op: Op, lhs: Expr, rhs: Expr) -> Expr {
    ...
    pub(crate) fn opt_field(name: &str, expr: Expr) -> FieldDef {
        FieldDef {
            name: name.into(),
            expr,
            optional: true,
    ...
    pub(crate) async fn json_eval_opt(expr: Expr) -> serde_json::Value {
        json_rows("{}", "", vec![opt_field("v", expr)], vec![]).await[0]["v"].clone()
    }

    /// Evaluate and expect an error, returning it.
    pub(crate) async fn json_eval_err(expr: Expr) -> String {
        let mut state = HostState::default();
    ...
    pub(crate) const MANGA_HTML: &str = r#"
        <html><body>
          <div class="container">
    ...
    pub(crate) const MANGA_JSON: &str = r#"
    {
      "data": [
        {
          "id": "manga-1",
