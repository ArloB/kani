...
pub enum Value {
    ...
    List(Vec<Value>),
    Null,
    HtmlElement { doc: Arc<Mutex<SafeHtml>>, node_id: NodeId },
...
impl Value {
    pub fn to_json(&self) -> Option<serde_json::Value> {
        match self {
            ...
            Value::List(items)   => Some(serde_json::Value::Array(
                items.iter().map(|v| v.to_json().unwrap_or(serde_json::Value::Null)).collect()
            )),
            Value::Null          => None,
    ...
    where F: FnOnce(String) -> Result<Value, String> {
        match self {
    ...
    pub fn into_list(self, op: &str) -> Result<Vec<Value>, String> {
        match self {
            Value::List(v) => Ok(v),
            Value::Json(serde_json::Value::Array(arr)) => Ok(arr.into_iter().map(Value::Json).collect()),
...
impl Env {
    pub fn new() -> Self { Env(Arc::new(HashMap::new())) }
    ...
    F: Fn(&'a Expr, Env) -> Pin<Box<dyn Future<Output = Result<Value, String>> + 'a>>,
{
    match expr {
        ...
        Expr::BinaryOperation { op, lhs, rhs } => Some((async {
            ...
            match (op, l, r) {
                ...
                | Op::Lt  | Op::Gt  | Op::Le  | Op::Ge, l, r) => numeric_op(op, l, r),
        ...
        Expr::Split { target, delimiter } => Some(
            recurse(target, env).await
                .and_then(|v| v.map_str("split", |s| Ok(Value::List(
                    s.split(delimiter.as_str()).map(|p| Value::Str(p.to_owned())).collect()
                ))))
        ),
        ...
        Expr::Replace { target, from, to } => Some(
            recurse(target, env).await
                .and_then(|v| v.map_str("replace", |s| Ok(Value::Str(s.replace(from.as_str(), to.as_str())))))
        ),
        ...
        Expr::Prepend { target, prefix } => Some((async {
            let s = recurse(target, env.clone()).await?;
            let p = recurse(prefix, env).await?;
            match (s, p) {
                (Value::Null, _) | (_, Value::Null) => Ok(Value::Null),
                (s, p) => Ok(Value::Str(p.into_str("prepend")? + s.into_str("prepend")?.as_str())),
            }
        }).await),

        Expr::Append { target, suffix } => Some((async {
            let s = recurse(target, env.clone()).await?;
            let x = recurse(suffix, env).await?;
            match (s, x) {
                (Value::Null, _) | (_, Value::Null) => Ok(Value::Null),
                (s, x) => Ok(Value::Str(s.into_str("append")? + x.into_str("append")?.as_str())),
            }
        }).await),
        ...
        Expr::Matches { target, pattern } => Some(
            recurse(target, env).await
                .and_then(|v| v.map_str("matches", |s|
                    get_or_compile_regex(pattern).map(|re| Value::Bool(re.is_match(&s)))
                ))
        ),

        Expr::Capture { target, pattern } => Some(
            recurse(target, env).await
                .and_then(|v| v.map_str("capture", |s|
                    get_or_compile_regex(pattern).map(|re| match re.captures(&s) {
                        None => Value::List(vec![]),
                        Some(caps) => Value::List(
                            (0..caps.len())
                                .map(|i| caps.get(i).map_or(Value::Null, |m| Value::Str(m.as_str().to_owned())))
                                .collect()
                        ),
                    })
                ))
        ...
        Expr::Fallback { target, default } => Some((async {
        ...
        Expr::ResolveUrl { target, base } => Some((async {
            let path     = recurse(target, env.clone()).await.and_then(|v| v.into_str("resolve_url"))?;
            let base_url = recurse(base, env).await.and_then(|v| v.into_str("resolve_url"))?;
        ...
        Expr::Concat(exprs) => Some((async {
            let mut s = String::new();
            for e in exprs {
                s.push_str(&recurse(e, env.clone()).await.and_then(|v| v.into_str("concat"))?);
            }
            Ok(Value::Str(s))
        }).await),

        Expr::List(exprs) => Some((async {
            let mut items = Vec::with_capacity(exprs.len());
            for e in exprs { items.push(recurse(e, env.clone()).await?); }
            Ok(Value::List(items))
        }).await),

        Expr::Map { target, transform } => Some((async {
            let items = recurse(target, env.clone()).await.and_then(|v| v.into_list("map"))?;
        ...
        Expr::FlatMap { target, transform } => Some((async {
            let items = recurse(target, env.clone()).await.and_then(|v| v.into_list("flat_map"))?;
        ...
        Expr::Filter { target, filter } => Some((async {
            let items = recurse(target, env.clone()).await.and_then(|v| v.into_list("filter"))?;
            let mut results = Vec::new();
            for (i, item) in items.into_iter().enumerate() {
                let mut inner = env.clone();
        ...
        Expr::Fold { target, transform, base } => Some((async {
            let items = recurse(target, env.clone()).await.and_then(|v| v.into_list("fold"))?;
        ...
        Expr::Let { name, value, body } => Some((async {
        ...
        Expr::If { condition, then, else_ } => Some((async {
        ...
        Expr::ToString { target } => Some((async {
            match recurse(target, env).await? {
                Value::Str(s)  => Ok(Value::Str(s)),
                Value::Int(i)  => Ok(Value::Str(i.to_string())),
                Value::Num(f)  => Ok(Value::Str(
                    if f.fract() == 0.0 && f.abs() < 1e15 { (f as i64).to_string() }
                    else { f.to_string() }
                )),
        ...
        Expr::Merge(lists) => Some((async {
            let mut merged = Vec::new();
            for list_expr in lists {
                let items = recurse(list_expr, env.clone()).await.and_then(|v| v.into_list("merge"))?;
                merged.extend(items);
            }
            Ok(Value::List(merged))
        }).await),

        Expr::Pref(key) => Some(Ok(
            env.get(&format!("$pref:{}", key)).cloned().unwrap_or(Value::Null)
        )),

        Expr::Format { template, args } => Some((async {
            let mut resolved = Vec::with_capacity(args.len());
            for arg in args {
                resolved.push(recurse(arg, env.clone()).await.and_then(|v| v.into_str("format"))?);
            }
...
pub async fn fetch_body(
    ...
    let rquest_url = url.to_string().parse::<rquest::Url>()
...
fn get_or_compile_regex(pattern: &str) -> Result<regex::Regex, String> {
    ...
    if let Some(re) = guard.get(pattern) { return Ok(re.clone()); }
...
fn numeric_op(op: &Op, l: Value, r: Value) -> Result<Value, String> {
    ...
    match op {
        Op::Add => Ok(if int { Value::Int(a as i64 + b as i64) } else { Value::Num(a + b) }),
        Op::Sub => Ok(if int { Value::Int(a as i64 - b as i64) } else { Value::Num(a - b) }),
        Op::Mul => Ok(if int { Value::Int(a as i64 * b as i64) } else { Value::Num(a * b) }),
        Op::Div => { if b == 0.0 { return Err("division by zero".into()); } Ok(Value::Num(a / b)) }
